...
import { FooterData } from "@/services/footer";
...
export default async function Footer({
  ...
  return (
    <footer
      className="bg-[#0a2240] text-white pt-16 pb-6 px-6"
      aria-label="Footer"
    >
            ...
            <div className="flex flex-col gap-2">
              {quickLinks.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className="text-sm text-white/50 hover:text-[#c9a84c] transition-colors duration-300 no-underline w-fit"
                >
                  {link.label}
                </Link>
              ))}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3">
              {phone && (
                <a
                  href={`tel:${phone}`}
                  className="flex items-center gap-3 text-sm text-white/50 hover:text-[#c9a84c] transition-colors duration-300 no-underline group"
              ...
              <div className="flex gap-3 mt-2">
                {socialLinks.map((social) => (
                  <a
                    ...
                    className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white/50 hover:bg-[#c9a84c] hover:text-white transition-all duration-300 hover:scale-110"
