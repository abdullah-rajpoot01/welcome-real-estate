  ...
  return (
    <div className={`fixed bottom-6 ${positionClass} z-50`}>
      ...
      >
        ...
        <Link
          href={phoneUrl}
          className="group flex items-center gap-3 bg-[#0a2240] hover:bg-[#081a31] text-white rounded-full pl-4 pr-2 py-2 shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl"
          onClick={() => setIsOpen(false)}
        >
      ...
      <button
        ...
        className={`relative flex items-center justify-center w-12 h-12 rounded-full shadow-lg transition-all duration-300  ${
          isOpen
            ? "bg-[#0a2240] hover:bg-[#061b35] rotate-90"
            : "bg-[#0a2240] hover:bg-[#061b35]"
      ...
      >
        ...
        {!isOpen && (
          <div className="absolute inset-0 rounded-full bg-[#0a2240] opacity-75 animate-ping" />
        )}
      </button>
    </div>
  );
